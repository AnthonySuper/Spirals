% !TEX encoding = UTF-8 Unicode
% !TEX TS-program = latexmk
\documentclass[a4paper]{book}
\usepackage[utf8]{inputenc}
\usepackage{todonotes}
\usepackage{censor}
\usepackage{enumitem}
\usepackage[breakable, skins]{tcolorbox}
\usepackage{imakeidx}
\usepackage{titlesec}
\usepackage{bold-extra}
\usepackage{wallpaper}


\makeindex

\usepackage[a4paper, margin=1in]{geometry}
\usepackage{multicol}
\usepackage{tocloft}

\usepackage{hyperref}

% SECTION HEADING FORMATTING
\definecolor{headerred}{HTML}{58180D}
\definecolor{headerunderlinegold}{HTML}{C9AD6A}

\titleformat{\chapter}[block]
{\bfseries\Huge\color{headerred}}
{Chapter \thechapter:}
{1ex}{}


\titleformat{\section}
  {\Large\bfseries\color{headerred}\scshape}
  {\thesection}{1em}{}
  
\titleformat{\subsection}
  {\normalfont\large\bfseries\color{headerred}}
  {\thesubsection}
  {1em}
  {}
  [{\color{headerunderlinegold}\nobreak\titlerule}]
  
\newcommand{\quoteauthor}[1]{\begin{flushright}--\textit{#1}\end{flushright}}

% BACKGROUND IMAGE

% GOD DESCRIPTION
\newenvironment{goddesc}{
\begin{tcolorbox}
\begin{description}[style=multiline,leftmargin=3cm]
\newcommand{\Domains}[1]{\item[Domains] ##1}
\newcommand{\Symbol}[1]{\item[Symbol] ##1}
\newcommand{\Level}[1]{\item[Level] ##1}
}{
\end{description}
\end{tcolorbox}
}

\newcommand{\FancyLetter}[1]{#1}

\newcommand{\VariantRule}[1]{
\addcontentsline{toc}{}{\protect\hspace{5em}\textit{Variant Rule: #1}\protect\cftdotfill{\cftdotsep} }%
\subsubsection*{Variant Rule: #1}\setcounter{tocdepth}{1}}%
\newcommand{\rolltablecolumnnumber}{1}

\newenvironment{rolltable}[2][\textheight]{
\begin{tcolorbox}[enhanced jigsaw, breakable, break at=#1]
\begin{center}
#2
\end{center}
\begin{footnotesize}
\begin{description}[style=multiline,leftmargin=1cm]
}{
\end{description}
\end{footnotesize}
\end{tcolorbox}
}


\definecolor{adventureideabg}{HTML}{aceca1}
\definecolor{adventureideaheaderbg}{HTML}{629460}
\definecolor{spellheaderbg}{HTML}{b1ede8}
\definecolor{spellbg}{HTML}{F0F7F4}




\newenvironment{itquote}
  {\begin{quote}\itshape}
  {\end{quote}\ignorespacesafterend}

\newcommand{\QuoteAttr}[1]{\begin{flushright} --- #1 \end{flushright}}
\newenvironment{itpars}
  {\par\itshape}
  {\par}
  
\newenvironment{adventureidea}[2][\textheight]
  {\begin{tcolorbox}[title=Adventure Idea: #2,colback=adventureideabg,colbacktitle=adventureideaheaderbg, breakable, break at=#1]

  }
  {\end{tcolorbox}}

\newenvironment{spell}[2][\textheight]
{\begin{tcolorbox}[title=#2,colback=spellbg,colbacktitle=spellheaderbg, breakable, break at=#1]
  \newcommand{\Type}[1]{\textit{##1}}
  \newcommand{\CastTime}[1]{\\ \textbf{Casting Time} ##1}
  \newcommand{\Components}[1]{\\ \textbf{Components:} ##1}
  \newcommand{\Range}[1]{\\ \textbf{Range:} ##1}
  \newcommand{\Duration}[1]{\\ \textbf{Duration:} ##1}
  }
  {\end{tcolorbox}}

\hypersetup{
    pdftex,
    colorlinks,
    citecolor=black,
    filecolor=black,
    linkcolor=black,
    urlcolor=blackf
}
\title{Spirals}
\author{Anthony Super}

\begin{document}
\setcounter{tocdepth}{1}

\maketitle

\TileWallPaper{2560px}{1090px}{../assets/bgimg.jpeg}

\todo[inline]{Add an actual hook to get people interested in the setting.}

Spirals is a Dungeons and Dragons 5\textsuperscript{th} edition campaign setting with an extremely strange physical structure, plentiful magic, and infinite extensibility.
This book is not a campaign module, but a description of the setting, how it works, and some rules you may need to be aware of.
This book mostly contains knowledge that the denizens of the world would know, so it is generally suitable to distribute to players, although that should be at the DM's discretion.

Although we don't provide a pre-made adventure in this book, we do provide "adventure ideas" throughout, to get your own creative juices flowing.
Hopefully they can be of some use.

Spirals is built in \LaTeX, and its source code is viewable online at https://github.com/AnthonySuper/Spirals.




\listoftodos
\tableofcontents 


\include{chapters/structure}

\include{chapters/core}
\include{chapters/root_tendrils}
\include{chapters/gods}
\include{chapters/items}
\include{chapters/life_in_the_world}
\include{chapters/cultures_of_the_world}

\printindex

\end{document}